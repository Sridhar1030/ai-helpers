...
for ROW in $(echo "$SELECTED" | python3 -c "
import json, sys
...
"); do
  ...
  PR_SCORE=$(echo "$ROW" | python3 -c "import json,sys; print(json.load(sys.stdin)['score'])")
...
for i, r in enumerate(succeeded):
    body += f\"| {i+1} | [#{r['number']}](https://github.com/${UPSTREAM_REPO}/pull/{r['number']}) | {r['title'][:60]} | {r['score']} |\n\"
...
'''
    for r in conflicts:
        body += f\"| [#{r['number']}](https://github.com/${UPSTREAM_REPO}/pull/{r['number']}) | {r['title'][:60]} | {r['score']} |\n\"
